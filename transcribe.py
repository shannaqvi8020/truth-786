...
def transcribe_audio(
    ...
    try:
        ...
        # Validate audio can be loaded before passing to Whisper
        if progress_callback:
        ...
        if not setup_ffmpeg_path():
            ...
            if not ffmpeg_path:
                raise RuntimeError(
                    "FFmpeg not found!\n\n"
                    "Please download FFmpeg from https://ffmpeg.org and either:\n"
        ...
        try:
            ...
            audio_array = whisper.audio.load_audio(audio_to_transcribe)
            if audio_array is None or len(audio_array) == 0:
                raise RuntimeError("Failed to load audio - file may be corrupted or unsupported format")
        except FileNotFoundError as fnf:
            # FFmpeg not found error
            raise RuntimeError(
                f"FFmpeg is required but not found.\n\n"
                f"Please download FFmpeg from https://ffmpeg.org and:\n"
                f"1. Place ffmpeg.exe in a 'ffmpeg_bin' folder next to TruthScript.exe\n"
                f"2. Or install FFmpeg and add it to your system PATH\n\n"
                f"Technical details: {fnf}"
            )
        except Exception as load_err:
            error_msg = str(load_err)
            if "WinError 2" in error_msg or "cannot find" in error_msg.lower():
                raise RuntimeError(
                    f"FFmpeg is required but not found.\n\n"
                    f"Please download FFmpeg from https://ffmpeg.org and:\n"
                    f"1. Place ffmpeg.exe in a 'ffmpeg_bin' folder next to TruthScript.exe\n"
            ...
            raise RuntimeError(f"Cannot read audio file: {load_err}")
        
        # Estimate duration for user feedback
        audio_duration_sec = len(audio_array) / 16000  # Whisper uses 16kHz sample rate
        audio_minutes = int(audio_duration_sec / 60)
        ...
        if progress_callback:
            progress_callback(0.72, f"Transcribing {audio_minutes}min audio ({time_msg}) - please wait...")
        
        # Transcribe with Whisper in a separate thread so we can show progress
        import threading
        import time
        ...
        def run_transcription():
            try:
                transcription_result[0] = model.transcribe(
                    audio_array,
        ...
        progress_value = 0.72
        progress_step = 0.12 / max(1, est_time * 6)  # Spread 0.72->0.84 over estimated time
        
        while not transcription_done.is_set():
            time.sleep(10)  # Update every 10 seconds
            if progress_value < 0.84:
                progress_value += progress_step
                elapsed = int((progress_value - 0.72) / progress_step * 10 / 60)
                if progress_callback:
                    progress_callback(progress_value, f"Transcribing... {elapsed}min elapsed")
        
        # Check for errors
        if transcription_error[0] is not None:
            err = transcription_error[0]
            if isinstance(err, AttributeError) and "'NoneType'" in str(err):
                raise RuntimeError(f"Audio processing failed - the file may be corrupted. Error: {err}")
